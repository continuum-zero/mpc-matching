...
pub async fn apply_swaps_round<E: MpcEngine, const N: usize>(
    ctx: &MpcExecutionContext<E>,
    elems: &mut [IntShare<E::Share, N>],
    instructions: &[MaybeSwap<E::Share>],
) {
    let results = {
        let elems = &*elems; // Borrow as immutable, so we can use it asynchronously below.
        join_circuits_all(instructions.into_iter().map(|inst| async move {
            (
                inst.index1,
                inst.index2,
                inst.condition
                    .swap_if_int(ctx, elems[inst.index1], elems[inst.index2])
                    .await,
            )
        }))
        .await
...
pub async fn apply_swaps<E: MpcEngine, const N: usize>(
    ctx: &MpcExecutionContext<E>,
    elems: &mut [IntShare<E::Share, N>],
    instructions: &[SwappingRound<E::Share>],
) {
    for swaps in instructions {
        apply_swaps_round(ctx, elems, swaps).await;
    }
}
...
pub async fn sort<E: MpcEngine, const N: usize>(
    ...
    while segment < n {
        ...
        while step >= 1 {
            ...
            apply_swaps_round(ctx, elems, &instructions).await;
